...
Uses Doctor API / document extraction for PDF-to-text when validating
document content (e.g. extracted text from court filings).
"""
...
def validate_court_document(state: dict) -> dict:
    ...
    3. Document text extraction (Doctor/fallback) when document_path/bytes provided
    4. LLM cross-reference for citation format and accuracy
    """
    ...
    # Optional: extract text from document for content validation
    extracted_text = ""
    if document_path or document_bytes:
        try:
            import sys, os
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
            from services.document_extractor import extract_document_text
            inp = document_path if document_path else document_bytes
            is_recap = "recap" in str(document_path or "").lower()
            ex = extract_document_text(inp, ocr_available=True, is_recap=is_recap)
            extracted_text = (ex.get("content") or "")[:3000]  # First 3k chars for context
            if extracted_text:
                raw_content = f"{raw_content}\n[Document content]: {extracted_text[:1500]}"
        except Exception:
            pass
